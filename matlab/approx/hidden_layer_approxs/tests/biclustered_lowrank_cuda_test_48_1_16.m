...
if do_compile
    ...
    cuda_vars.B_Y_conv = rank; 
    cuda_vars.B_X_conv = 16;
	%cuda_vars.filtersPerThread =16;
    cuda_vars.imgsPerThread_conv = 4; %numImages % 128 == 0 ? 4 : numImages % 6/4 == 0 ? 2 : 1;
    cuda_vars.scale = 0;
    cuda_vars.checkImgBounds = mod(numImages, cuda_vars.B_X_conv*cuda_vars.imgsPerThread_conv) ~= 0;

    cuda_vars.B_Y_comb = 32;
    cuda_vars.B_X_comb = 32;
    cuda_vars.imgsPerThread_comb = 1;
    
    % replace template variables, and compile
    fid_read = fopen(strcat(root_path, 'gpu/src/biclustered_hidden_template-48_1_16.cuh'), 'r');
...
% Check correctness
X = randn(numImages, N, N, Nin); %permute(repmat(randn(N), [1, 1, 128, Nin]), [3, 1, 2, 4]); %randn(numImages, N, N, Nin);
W = randn(Nout, K, K, Nin);
out_partial = ones(numImages, M, M, rank, numClustIn, numClustOut);
...
C_(CopyToGPU, gids.X,  single(X));
C_(CopyToGPU, gids.out,  out_);
C_(CopyToGPU, gids.out_partial,  single(out_partial));

lapse2 = [];
for t=1:num_runs
	C_(StartTimer);
	C_(approx_pointer, gids.X, gids.F, gids.C, gids.XY, gids.out, gids.out_partial, gids.inPerm, gids.outPerm, N, Nin, Nout, K, stride, padding);
	lapse = C_(StopTimer); 
	lapse2 = [lapse2, lapse];
end
