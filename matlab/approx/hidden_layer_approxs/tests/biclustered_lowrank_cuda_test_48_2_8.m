...
clearvars -except speedup_vs_rank ranks;
global root_path
init();
...
if do_compile
    ...
    fid_read = fopen(strcat(root_path, 'gpu/src/biclustered_hidden_template-48_2_8.cuh'), 'r');
...
for t=1:num_runs
	C_(StartTimer);
	C_(approx_pointer, gids.X, gids.F, gids.C, gids.XY, gids.out, gids.inPerm, gids.outPerm, N, Nin, Nout, K, stride, padding);
	lapse = C_(StopTimer); 
	lapse2 = [lapse2, lapse];
end
