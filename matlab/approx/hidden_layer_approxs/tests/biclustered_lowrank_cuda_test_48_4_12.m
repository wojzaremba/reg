...
if do_compile
    ...
    cuda_vars.B_X_comb = 32;
    cuda_vars.B_Y_comb = rank * numClustOut;
    cuda_vars.imgsPerThread_comb = 4; % cuda_vars.imgsPerThread_comb * cuda_vars.B_X_comb = numImages = 128
    cuda_vars.B_Y_conv = 4; 
    cuda_vars.B_X_conv = 32;
	cuda_vars.ranksPerThread = 1;
    cuda_vars.imgsPerThread_conv = 4; %numImages % 128 == 0 ? 4 : numImages % 6/4 == 0 ? 2 : 1;
    cuda_vars.scale = 0;
    cuda_vars.checkImgBounds = 0;
    cuda_vars.totalRank = rank;
...
in_partial = zeros(numImages, N, N, rank, cuda_vars.numComputeIn, numClustOut);
out_partial = zeros(numImages, M, M, rank, cuda_vars.numComputeIn, numClustOut);
...
out_partial = reshape(C_(CopyFromGPU, gids.out_partial), size(out_partial));
