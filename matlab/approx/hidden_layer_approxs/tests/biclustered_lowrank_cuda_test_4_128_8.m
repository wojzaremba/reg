...
if do_compile
    ...
    cuda_vars.outClustPerBlock = 8;
    cuda_vars.B_X_comb = 16;
    cuda_vars.B_Y_comb = rank;
    cuda_vars.B_Z_comb = 4;
    cuda_vars.numComputeOut = 16;
    cuda_vars.imgsPerThread_comb = numImages / cuda_vars.B_X_comb; % cuda_vars.imgsPerThread_comb * cuda_vars.B_X_comb = numImages = 128
    cuda_vars.B_X_conv = 32;
    cuda_vars.B_Y_conv =16;
    cuda_vars.imgsPerThread_conv = 2; %numImages % 128 == 0 ? 4 : numImages % 6/4 == 0 ? 2 : 1;
    cuda_vars.scale = 0;
    cuda_vars.checkImgBounds = 0;
    cuda_vars.rank = rank;

    % replace template variables, and compile
    fid_read = fopen(strcat(root_path, 'gpu/src/biclustered_hidden_template-4_128_8.cuh'), 'r');
...
in_partial = zeros(numImages, N, N, rank, cuda_vars.numClustIn, cuda_vars.numComputeOut);
%out_partial = zeros(numImages, M, M, rank, cuda_vars.numClustIn, numClustOut);
...
C_(CopyToGPU, gids.X,  single(X));
C_(CopyToGPU, gids.out,  out_);
C_(CopyToGPU, gids.in_partial, in_partial);
C_(CopyToGPU, gids.out_partial,  single(randn(4)));

lapse2 = [];
for t=1:num_runs
	C_(StartTimer);
	C_(approx_pointer, gids.X, gids.F, gids.C, gids.XY, gids.out, gids.in_partial, gids.out_partial, gids.inPerm, gids.outPerm, N, Nin, Nout, K, stride, padding);
	lapse = C_(StopTimer); 
	lapse2 = [lapse2, lapse];
end
partial = reshape(C_(CopyFromGPU, gids.in_partial), size(in_partial));
...
speedup = lapse1(2:end) ./ lapse2(2:end);
