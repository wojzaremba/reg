function [outlabel,outU,outV] = litekflats(X, k, q)
% Perform k-q flats clustering
%   X: d x n data matrix
%   k: number of seeds
%   q: dimension of each subspace
...
for j=1:outiters
    ...
    [label] = constrained_assignment(X, m,n/k, 0, Xnorms);
    [U,V] = estimate_q_flats(label, X, q);
    ...
    while any(label ~= last) & iters < maxiters
        ...
        last = label;
        %[~,label] = max(bsxfun(@minus,m'*X,dot(m,m,1)'/2),[],1); % assign samples to the nearest centers
        [label, ener] = constrained_assignment(X, U, n/k,1,Xnorms);
...
function [U,V] = estimate_q_flats(label, X, q)
...
function [w]=qflats_assignment(X, U,Xnorms)
%I return for each pair X_i and each subspace V_j the distance || X_i - V_j * V_j' * X_i ||^2
for i=1:size(U,2)
%UU = U{i} * U{i}';
%Xbis = X - UU * X;
%dists = sum(Xbis.^2);
...
function [out,ener]=constrained_assignment(X, C, K, type, Xnorms)
...
w=qflats_assignment(X,C,Xnorms);
end
