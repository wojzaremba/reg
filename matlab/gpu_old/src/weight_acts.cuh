/* 
 * Cepyright (c) 2011, Alex Krizhevsky (akrizhevsky@gmail.com)
 * All rights reserved.
 */


#ifndef WEIGHTACTS_H_
#define WEIGHTACTS_H_
...
void _weightActs(NVMatrix& images, NVMatrix& hidActs, NVMatrix& targets,
    ...
    assert_(numGroups > 1 || (numImgColors > 0 && (numImgColors <= 3 || numImgColors % 4 == 0)));
    assert_(numGroups == 1 || numFilterColors % 4 == 0);
    ...
    assert_(numModules % partialSum == 0);
    assert_(hidActs.getNumCols() == numImages);
    ...
    assert_(numModules * numFilters == hidActs.getNumRows());

    assert_(!images.isTrans());
    assert_(!hidActs.isTrans());
    assert_(hidActs.isContiguous());
    ...
    if (numFilterColors > 3) {
        ...
        assert_(numGroups == 1); // Just for sanity
    ...
    if (scaleTargets == 0) {
        ...
        assert_(targets.getNumRows() == (numModules/partialSum) * numFilterColors*filterPixels);
        assert_(targets.getNumCols() == numFilters);
    }
    if (numFilterColors > 3) {
...
void _weightActsSparse(NVMatrix& images, NVMatrix& hidActs, NVMatrix& targets, int* dColorIndices,
    ...
    assert_(numGroups > 1);
    assert_(numImgColors % numFilterColors == 0);
    assert_((numFilterColors * numGroups) % numImgColors == 0);
    assert_(numFilters % (16*numGroups) == 0);
    assert_(numFilterColors % 4 == 0);
    assert_(imgSizeY * imgSizeX == imgPixels);
    assert_(images.getNumRows() == imgPixels * numImgColors);
