...
void ConvActUndo(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  assert_((nrhs == 11) && (nlhs == 0));
	NVMatrix* hidActs = getMatrix(prhs[1]);
  ...
  convImgActs(*hidActs, *filters, *outGrad, imgSizeY, imgSizeY,
        numModulesY, -paddingStart, moduleStride, numImgColors, 1, 0.0, 1.0);
  convWeightActs(*images, *hidActs, *weightGrad,                                           
...
void ConvResponseNormCrossMapUndo(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	float k = (float)mxGetScalar(prhs[8]);
	float alpha = (float)mxGetScalar(prhs[9]);
	float beta = (float)mxGetScalar(prhs[10]);
  convResponseNormCrossMapUndo(*grads, *denoms, *inputs, *acts, *targets, numFilters, n, k, alpha, beta, 0.0, 1.0);
	inputs->transpose();
	denoms->transpose();
	targets->transpose();
  grads->transpose();
