classdef Maha < Layer %% Same as softmax except for backward gradient
    properties
        beta
        d
    end
    ...
    methods
        ...
        function BPgpu(obj)
            assert(obj.beta == 1);
            global plan
            correct = plan.input.cpu.vars.Y;
            bs = plan.input.batch_size;
            pred = C_(CopyFromGPU, obj.gpu.vars.pred);
            [~, idx] = sort(pred, 2, 'descend');
            worst_mask = zeros(size(plan.input.cpu.vars.Y));
            for i = 1 : bs
                cc = find(correct(i, :));
                id = idx(i, :);
                id = id(id ~= cc);
                worst_mask(i, id(obj.d)) = 1;
            end
            dX = pred - worst_mask;
            C_(CopyToGPU, obj.gpu.dvars.X, single(dX));
        ...
        function BP(obj)
            ...
            correct = plan.input.cpu.vars.Y;
            bs = plan.input.batch_size;
            [~, idx] = sort(obj.cpu.vars.pred, 2, 'descend');
            worst_mask = zeros(size(plan.input.cpu.vars.Y));
            for i = 1 : bs
               cc = find(correct(i, :));
               id = idx(i, :);
            ...
            obj.cpu.dvars.X = (obj.cpu.vars.pred - worst_mask) / obj.beta; 
        end                
