 ...
 */
 void _filterActs(NVMatrix& images, NVMatrix& filters, NVMatrix& targets,
    ...
    assert_(filters.getNumRows() == filterModuleMult * numFilterColors * filterPixels);
    ...
    if (imgsPerThread == 4) {
        if (numImgColors <= 3) {
            assert_(numGroups == 1); // It has to be based on above definitions, but just to be sure.
